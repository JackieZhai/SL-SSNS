...
class Train(Dataset):
	def __init__(self, cfg):
		...
		if cfg.DATA.unlabel_dataset ==  'cremi-bc-all':
			self.u_sub_path = 'cremi-r/all'
			self.u_train_datasets = ['cremiB-top.tif', 'cremiC-top.tif']
		elif cfg.DATA.unlabel_dataset ==  'cremi-bc-all-p1':
			self.u_sub_path = 'cremi-r/all'
			self.u_train_datasets = ['cremiB+-top.tif', 'cremiC+-top.tif']
		elif cfg.DATA.unlabel_dataset ==  'cremi-bc-all-p2':
			self.u_sub_path = 'cremi-r/all'
			self.u_train_datasets = ['cremiB+-bottom.tif', 'cremiC+-bottom.tif']
		elif cfg.DATA.unlabel_dataset ==  'cremi-bc-all-p3':
			self.u_sub_path = 'cremi-r/all'
			self.u_train_datasets = ['cremiB+-bottom.tif', 'cremiC+-top.tif']
		else:
			raise AttributeError('No this dataset type!')
		...
		self.u_folder_name = os.path.join(cfg.DATA.data_folder, self.u_sub_path)
		...
		self.u_dataset = []
		for k in range(len(self.u_train_datasets)):
			print('load unlabeled ' + self.u_train_datasets[k] + ' ...')
			# load raw data
			data = imageio.volread(os.path.join(self.u_folder_name, self.u_train_datasets[k]))
			self.u_dataset.append(data)
		
		for k in range(len(self.u_dataset)):
			if self.u_dataset[k].shape[0] == 65:
				self.u_dataset[k] = np.pad(self.u_dataset[k], ((self.net_padding[0], self.net_padding[0]),
														(self.net_padding[1], self.net_padding[1]),
														(self.net_padding[2], self.net_padding[2])), mode='reflect')
			else:
				self.u_dataset[k] = np.pad(self.u_dataset[k], ((2, 3),
														(self.net_padding[1], self.net_padding[1]),
														(self.net_padding[2], self.net_padding[2])), mode='reflect')
		# the training dataset size
		self.u_data_shape = list(self.u_dataset[0].shape)
		print('unlabeld shape: ', self.u_data_shape)
	...
	def __getitem__(self, index):
		...
		####################### get unlabeled 
		u_k = random.randint(0, len(self.u_train_datasets)-1)
		u_used_data = self.u_dataset[u_k]
		random_z = random.randint(0, self.u_data_shape[0] - self.crop_from_origin[0])
		random_y = random.randint(0, self.u_data_shape[1] - self.crop_from_origin[1])
		random_x = random.randint(0, self.u_data_shape[2] - self.crop_from_origin[2])
		imgs2 = u_used_data[random_z:random_z + self.crop_from_origin[0], \
		     	random_y:random_y + self.crop_from_origin[1], \
				random_x:random_x + self.crop_from_origin[2]].copy()
		...
		imgs2, _, _, _, _  = self.apply_perturbations(imgs2, imgs2) # SM真的适合吗
        ####################### make mask
		...
		# copy-paste
			...
			if random.random() > 0.5:
				...
				mask = np.zeros([18, 160, 160])
				mask[:,random_y_mask:random_y_mask+90, random_x_mask:random_x_mask+90] = 1
			...
			mask = np.zeros([18, 160, 160])
			mask[:,magic_order[0][0]:magic_order[0][1],magic_order[0][2]:magic_order[0][3]] = 1
			mask[:,magic_order[1][0]:magic_order[1][1],magic_order[1][2]:magic_order[1][3]] = 1
			imgs = mask * imgs1 + (1-mask) * imgs2
			

		# cv2.imwrite('./data_temp/imgs%d.png'%index, (255 * imgs1[1]).astype('uint8'))
		# cv2.imwrite('./data_temp/imgs2.png', (255 * imgs2[1]).astype('uint8'))
		# cv2.imwrite('./data_temp/la%d.png'%index, (255 * lb_affs1[1][1]).astype('uint8'))
		# raise EnvironmentError
	
		imgs = imgs[np.newaxis, ...]
		imgs = np.ascontiguousarray(imgs, dtype=np.float32)
		lb_affs = np.ascontiguousarray(lb_affs1, dtype=np.float32)
		gt_imgs2 = gt_imgs2[np.newaxis, ...]
		gt_imgs2 = np.ascontiguousarray(gt_imgs2, dtype=np.float32)
		mask = mask[np.newaxis, ...]
		mask = np.concatenate([mask, mask, mask], axis=0)
		mask = np.ascontiguousarray(mask, dtype=np.float32)
		return imgs, lb_affs, gt_imgs2, mask
		
        # if random.random() > 0.5:
		# 	imgs = mask * imgs1 + (1-mask) * imgs2
		# 	flag = 1
		# else:
		# 	imgs = mask * imgs2 + (1-mask) * imgs1
		# 	flag = 0
		# extend dimension
	...
	def perturbations_init(self):
		...
		self.per_cutout = Cutout(model_type='mix')
...
if __name__ == '__main__':
	...
	np.random.seed(seed)
	random.seed(seed)
	cfg_file = '/data2/zhangyc/semi-cross/config/seg_snemi3d_d5_1024_u200.yaml'
	with open('./config/' + cfg_file, 'r') as f:
		cfg = AttrDict( yaml.load(f) )
	
	out_path = '/data2/zhangyc/semi-cross/data_temp'
